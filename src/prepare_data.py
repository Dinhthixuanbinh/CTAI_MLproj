...
def create_multi_task_datasets(base_dir: str = '/kaggle/input/copy-crema-d/cremad'):
    ...
    classification_save_path = os.path.join('/kaggle/working/', 'cremad_classification.csv')
    ...
    logger.info(f"Successfully created classification CSV file with {len(df_classification)} samples.")
    logger.info(f"Successfully created contrastive CSV file with {len(df_contrastive)} pairs.")
